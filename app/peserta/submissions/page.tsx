...
export default function SubmissionsPage() {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      // Get user profile to access assigned groups
      const userProfile = await getProfile()
      const userGroups = userProfile.userGroups || []
      
      let allSubmissions: Submission[] = []
      
      // Fetch sessions for each assigned group
        ...
        try {
          ...
          if (session) {
            allSubmissions.push({
              ...
              groupName: session.groupName || userGroup.group?.groupName || `Group ${session.groupId}`,
          ...
          console.log(`Failed to fetch session for group ${userGroup.groupId}:`, error)
      ...
      if (allSubmissions.length === 0 && userGroups.length > 0) {
        allSubmissions = userGroups.map((userGroup: any, index: number) => ({
          ...
          groupName: userGroup.group?.groupName || `Group ${userGroup.groupId}`,
                ...
                <CardContent className="p-6">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        {submission.status === 'draft' ? 'Start Assessment' : 'Continue Assessment'}
                      </Button>
                    </div>
                  </div>
                </CardContent>
